# -*- coding: utf-8 -*-
# GOST-ASSISTANT v3.4 — «академический эксперт»: реальные источники из 10+ каталогов,
...
def _strip_html(raw: str) -> str:
    """Очень лёгкая HTML→text оч��стка без внешних зависимостей."""
    if not raw:
        return ""
...
def _filter_relevant_source_records(records: list[dict], topic: str, subject: str, limit: int, doc_type: str = "") -> list[dict]:
    ...
    # в следующих query-вариантах. Ранний ��рез до 60–120 записей отбрасывал их.
    ...
    def _add(candidates: list[dict], require_topic_match: bool = False) -> None:
        for r in candidates:
            ...
            if not k or k in selected_keys:
                continue
            # FIX: Проверяем что ис��очник реально по теме. Для тем с именами
    ...
    # Сначала берём strong с пров��ркой по теме
...
def _format_source_record(record: dict) -> str:
    ...
    # «[URL не указан]». По ГОСТ 7.32 ис��очник без электронного адреса
...
async def generate_chapter_titles(
    ...
    return StructureGenerator.get_default_chapter_titles(doc_type, topic, num_chapters)
...
def _dedupe_chapter_titles(chapters: list[dict]) -> list[dict]:
    """Убирает дублирующиеся главы и подглавы.

    LLM иногда генерирует:
    - одинаковые или почти одинаковые подглавы («1.1 Понятие» и «1.1 Понятие проблемы»)
    - одинаковые главы (две главы с одним заголовком)
    Оставляем только уникальные, сохраняя порядок.
    """
    seen_chapter_norms: set[str] = set()
    seen_subtitles: set[str] = set()
    result: list[dict] = []
    for ch in chapters:
        title = ch.get("title", "")
        # Нормализуем заголовок главы для проверки дублей
        ch_norm = re.sub(r"^\d+\.?\s*", "", title).strip().lower()
        ch_norm = re.sub(r"[^а-яёa-z0-9]", "", ch_norm)
        if ch_norm and ch_norm in seen_chapter_norms:
            print(f"[STRUCT] Дублирующая глава удалена: {title}")
            continue
        if ch_norm:
            seen_chapter_norms.add(ch_norm)
        raw_subs = ch.get("subs", [])
        unique_subs: list[str] = []
        for sub in raw_subs:
            # Нормализуем для сравнения: убираем номер, точки, приводим к нижнему регистру
            norm = re.sub(r"^\d+(?:\.\d+)*\.?\s*", "", sub).strip().lower()
            norm = re.sub(r"[^а-яёa-z0-9]", "", norm)
            if norm and norm not in seen_subtitles:
...
def _default_chapter_titles(doc_type: str, topic: str, num_chapters: int) -> list[dict]:
    ...
    # ГОСТ 7.32-2017: без слова «Глава» и без точки после номера раздел��
    defaults = [
        {
...
def build_prompts(
    ...
    # Это критично: ��ез этого «цель и задачи» во введении живут отдельно
    ...
    if _tasks_list:
        ...
        _tasks_block = (
            "\n\nОБЯЗАТЕЛЬНО сформулируй РОВНО эти задачи (можно сл��гка "
    ...
    prompts["intro"] = strict_prompt(
        ...
        f"Абзац 3 — цель и 3–5 з��дач исследования (задачи перечисли внутри абзаца через «во-первых», «во-вторых»), "
    ...
    # Заключение НЕ включаем �� batch — оно генерируется ПОСЛЕ всех глав
...
async def generate_text_blocks(
    ...
    # Подтягиваем реальные ��сточники и материалы с URL до построения промптов.
    ...
    # него: fetch_rows каталогов увеличен до 150 н�� каждый query-вариант
    ...
    # ── Базовые правила для всех ти��ов ──
    style_sys = (
    ...
    if doc_type == "esse":
        style_sys += (
            ...
            "Ни��аких «мы рассмотрели», «было установлено» — только «я». "
    ...
    style_sys += (
        ...
        "НЕ используй `[N]` без страницы. Н��КОГДА не оставляй `[N, с.` "
    ...
    async def _gen_one_block(key, prompt):
        ...
        if key != "literature":
            ...
            # Повторная генерация ��ри пустом/слишком коротком ответе.
            ...
            # Дозаполняем если текст короче цели на 25%+ (к��оме литературы).
    ...
    # conclusion генерируется ОТДЕЛЬНО ниже, посл�� глав — его здесь нет.
    # ═══════════════════════════════════════════════════════════
    ...
    # ── Жа��рово-зависимый промпт заключения ──
    if doc_type == "esse":
        conc_prompt = (
        ...
        conc_prompt = (
            ...
            f"5. Используй сноски тольк�� с номерами страниц: [1, с. 45], [2, с. 120].\n"
    ...
    # выкидывает п��едложения с фактами, которых нет в главах.
    ...
    for key in list(parts.keys()):
        ...
        # Последний обязательный проход: ни о��ной ссылки вида [1] без страницы.
        parts[key] = _fill_missing_pages(parts[key])
...
_COMMON_RU_FIRST_NAMES = {
    ...
    "б��гдан", "борис", "вадим", "валентин", "валерий", "василий", "виктор",
...
def _word_looks_like_verb(word: str) -> bool:
    ...
    # 1) Стоп-слова — короткие служебные русские слова, з��ведомо не имя.
...
_INSTITUTION_WHITELIST = {
    "МГУ", "СПбГУ", "МГИМО", "МФТИ", "МИФ��", "ВШЭ", "НИУ", "МГТУ",
...
def _validate_fio(text: str, *, kind: str = "ФИО") -> tuple[bool, str]:
    ...
    Правила:
      - 2 или 3 слова (русская и м��ждународная практика).
      - Каждое слово ≥ 2 букв.
    ...
    # Каждое русское слово должно содержать хотя бы одну гласную
    # (защита от «��рш крн» и подобного мусора)
...
def _clean_title_page_garbage(text: str) -> str:
    ...
    # аббревиатуры («ООО», «Г��У», «ВУЗ», «МГУ», «ФГБОУ» и т. п.).
...
def _validate_topic_not_truncated(topic: str, max_display_len: int = 80) -> str:
    ...
    # и после��нее слово короче 2 букв) �� убираем хвост
    words = topic.split()
...
def _fill_missing_pages(text: str, global_page_map: dict | None = None) -> str:
    ...
    def _get_page(n: str) -> str:
        # Если есть ��еальная страница из текст��/глобального контекста
...
def _ensure_block_terminates(text: str) -> str:
    ...
    срезаем хвосты вида «… под руководством А.В.» или «… выделить ��ж.»,
        ...
        if last_term > 0:
            ...
            if not re.search(r'[сСcC]\.\s*\d', around):
                if last_term >= para_cutoff:
                    ...
                    continue
        # Ничего не помогло — добавляем ��очку
        cleaned_paragraphs.append(para + ".")
...
_GENERIC_ACADEMIC_WORDS = frozenset({
    # глаголы и формы
    "является", "являются", "представля��т", "представляют", "позволяет",
    ...
    "проведенное", "��ыявлено", "выявлены", "установлено", "установлены",
    ...
    "уникальным", "уникальный", "уник��льная", "уникальное",
    ...
    "ключевым", "ключевая", "ключевое", "основным", "основ��ая", "основное",
...
def _strip_forbidden_openers(conc_text: str) -> str:
    ...
    в ��ачале КАЖДОГО абзаца заключения и сразу после `. ` внутри
...
def _chapter_summary_from_parts(parts: dict, idx: int) -> str:
    ...
    if idx == 1:
        ...
        return ("раскрыты исходные теоретическ��е положения, необходимые для "
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    for pos, idx in enumerate(chapter_idxs, start=1):
        ...
        bridges = [
            ...
            "Обоснованные положен��я вносят вклад в развитие научного дискурса по теме, связывая конкретные факты с теоретическими обобщениями.",
        ]
...
def setup_gost_page(doc: Document, gost: dict) -> None:
    ...
    for sec in doc.sections:
        # python-docx по ��молчанию создаёт Letter (21,59×27,94 см).
...
def _heading_compare_key(text: str) -> str:
    """Ключ сравнения з��головков: без markdown, номера, знаков препинания."""
...
def _render_gost_formula(doc: Document, ch: str, gost: dict) -> None:
    """ГОСТ 7.32-2017: формула выключкой по центру ��троки, номер (N) у правого поля."""
    font = gost.get("font_name", "Times New Roman")
    base = int(gost.get("font_size", 14))
...
def _is_safe_real_photo_meta(*values: str) -> bool:
    ...
    if any(w in blob for w in _BAD_IMAGE_WORDS):
        return False
    # Если явно указано, ��то это фото/портрет/official — отлично.
...
async def search_openverse_images(query: str, limit: int = 3, source: str = "") -> list[dict]:
    """Бесплатный официальный Openverse API: реальные ф��то/изображения Creative Commons без ключа."""
    if not ENABLE_IMAGE_SEARCH or limit <= 0:
        return []
    query = " ".join((query or "").split()).strip()
    if not query:
...
def build_docx_bytes(
    ...
    def _maybe_add_image_after_block(block_title: str, block_level: int, is_bib_block: bool) -> None:
        ...
        if block_level != 1:
            return
        # Находим последний абзац тела главы, чтоб�� добавить в текст отсылку
        ...
        if subblocks:
            ...
            for sub_idx, (sub_title, sub_text) in enumerate(subblocks):
                ...
                if sub_text:
                    ...
                    # ═══ Подглава без текста — вставляем аварийную заглушку ПРЯМО �� DOCX ═══
...
def estimate_docx_pages(docx_path: str) -> int | None:
    ...
    # для документа из 25 000 знаков с 6+ разрывами эстиматор показыва�� ~16,
...
async def measure_pages_async(docx_path: str, work_dir: str) -> int | None:
    ...
    1. LibreOffice → PDF + PyMuPDF/PyPDF2 — самый честный для нашего пайплайна:
       ...
       не было расхождения между «помери��и в Aspose, обновили в LO».
...
def _trim_blocks_by_chars(blocks: list[tuple], chars_to_remove: int,
    ...
    for b in blocks:
        ...
        if text_len < 100 and not any(w in (b[0] or "").upper() for w in ("ЛИТЕРАТ", "ИСТОЧНИК", "БИ��ЛИОГРАФ")):
...
async def _expand_blocks_by_chars(
    ...
    # в дописанных куск��х получал страницу из ран��е увиденных `[N, с. K]`.
    _global_pm: dict = {}
    for _b in blocks:
...
def kb_mode() -> InlineKeyboardMarkup:
    ...
    b.button(text="⭐ Платный ��ежим  (безлимит)", callback_data="mode_paid")
    b.adjust(1)
    return b.as_markup()
...
def kb_image_source() -> InlineKeyboardMarkup:
    ...
    (user-patch): авто-поиск убран; загрузка ��обственных фото-файлов
...
def kb_topic_relevance_options(
    ...
    """Кнопки п��д предупреждением о неочевидной связи темы и дисциплины."""
    rows: list[list[InlineKeyboardButton]] = []
...
async def _extract_docx_text(
    ...
    полностью терялись все изображения исходного файла (документ чи��ался
...
def _build_edited_docx_bytes(corrected_text: str, gost: dict, source_images: list[dict] | None = None) -> bytes:
    ...
    (извлечённые в _extract_docx_text) равномерно р��спределяются по телу
...
async def h_doc_type(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # ВАЖНО: о��рабатываем more/back ДО проверки membership, иначе
...
async def h_topic(message: Message, state: FSMContext) -> None:
    ...
    if len(topic) < 10 or len(re.findall(r"[А-Яа-яЁёA-Za-z]{3,}", topic)) < 2:
        await message.answer(
            ...
            "<i>Например: «Влияние презид��нтства Дональда Трампа на электоральную географию США»</i>",
            parse_mode="HTML",
            reply_markup=kb_back_cancel(),
        )
        return
...
async def h_source_no(cb: CallbackQuery, state: FSMContext) -> None:
    """Поль��ователь выбрал 'Нет — только по теме'"""
...
async def h_author(message: Message, state: FSMContext) -> None:
    raw = (message.text or "").strip()
    ok, value = _validate_fio(raw, kind="Ф��О автора")
    if not ok:
        await message.answer(
            value,
            parse_mode="HTML",
...
async def h_own_image_links(message: Message, state: FSMContext) -> None:
    ...
    if rejected:
        lines.append(f"⚠️ Не удалось ск��чать или это не картинка: {len(rejected)} шт.")
...
async def h_regenerate(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # Восстанавливаем пар��метры (и сохраняем ��ам job, чтобы можно было
        ...
        try:
            ...
            if data.get("include_images"):
                ...
                if not data.get("images"):
                    ...
                    # Адаптивная ширина: зав��сит от количества изображений и страниц,
            ...
            # ── LibreOffice (финальная конвертация — ��бновит TOC и поля PAGE) ──
